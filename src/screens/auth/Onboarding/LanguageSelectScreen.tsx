...
/** Runs post-signup, first in the sequence (see RootNavigator), before
 *  CountrySelectScreen — language is auto-detected from the device as the
 *  starting point (see LanguageProvider), this just lets the user confirm
 *  or override that guess explicitly. Picking Togo on the next screen can
 *  still auto-switch this to French regardless of what's chosen here. */
export function LanguageSelectScreen() {
  const navigation = useNavigation<any>();
  const route = useRoute<any>();
  const nextScreen = route.params?.nextScreen ?? 'PostAuthCountrySelect';
